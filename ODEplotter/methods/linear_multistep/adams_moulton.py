...
def adams_moulton(
    ...
    while True:
        ...
        next_y_guess = y + h * weighted_sum(derivatives, predictor_weights)
...
class AdamsMoulton(SolutionMethod):
    ...
    def _validate(self):
        ...
        self.predictor_weights = to_weight_array(self._predictor_weights)

        del self._weights, self._predictor_weights
        self.validated = True

    def _prepare_arguments(self, derivative: DerivativeFunction, t0: Time, y0: Vector, h: Time, use_jit: bool):
        return (derivative, t0, y0, h, self.weights, self.predictor_weights)
